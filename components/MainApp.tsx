...
type ActiveView = 'home' | 'myfinpay' | 'qr' | 'inbox';
...
const MainApp: React.FC<MainAppProps> = ({ user, onLogout, onUserUpdate, toggleTheme, isDarkMode }) => {
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      case 'qr':
        // This would ideally open the camera directly, but for now, it's a placeholder screen
         return <div className="p-4 text-center text-gray-500">{t('mainApp.qrScan')}</div>;
      default:
        return null;
    }
  };
      ...
      {user.userType !== UserType.ADMIN && (
          <footer className="fixed bottom-0 w-full max-w-sm mx-auto bg-white/90 dark:bg-gray-950/90 backdrop-blur-sm border-t dark:border-gray-800 shadow-t-lg">
              <nav className="flex justify-around items-center h-16">
                  ...
                  <NavItem icon={<QrCode />} label={t('mainApp.qrScan')} view="qr" />
